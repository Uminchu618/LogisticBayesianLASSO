...
def run_simulation(seed, X, y):
    # np.random.seed(1) とデータ生成部分を削除し，受け取ったX,yを使用
    np.random.seed(seed)
...
def run_simulation_lambda(args, X, y):
    seed, lambda1 = args
    np.random.seed(seed)
...
def run_lasso_path_simulation(X, y):
    ...
    for lam in lambda_grid:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            beta_estimates = list(
                tqdm(
                    executor.map(partial(run_simulation_lambda, X=X, y=y), seeds),
                    total=n_runs,
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        beta_estimates = list(
            tqdm(
                executor.map(partial(run_simulation, X=X, y=y), range(n_runs)),
                total=n_runs,
                desc="シミュレーション進行中",
            )
        )
